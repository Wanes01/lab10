...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
        final List<Optional<T>> lout = new ArrayList<>(list.size());
        list.forEach(el -> lout.add(pre.test(el) ? Optional.of(el) : Optional.empty()));
        return lout;
    }
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((key, val) -> result.put(key, val.isPresent() ? val.get() : def.get()));
        return result;
    }
